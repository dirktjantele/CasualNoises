...
void createInitialTLVs ( QueueHandle_t TLV_DriverQueue )
{
	...
	if ( index == 0 )
	{
		gSystemSettings.calibrationData.reveserved_1 = '>';			// Just an eye catcher
		gSystemSettings.calibrationData.isCalibrated = 'f';
		gSystemSettings.calibrationData.reveserved_2 = 0x2d3c;		// '<-'

		// Set all potentiometers full range, note that the sliders give an inverted value
		for (uint32_t i = 0; i < NO_OF_ADC_MULTIPLEXERS; ++i)
		{
			for (uint32_t j = 0; j < NO_OF_ADC_MULTI_CHANNELS; ++j)
			{
				if ( i == 0 )
				{
					gSystemSettings.calibrationData.minValues[i][j] = 0x00000000;		// Potentiometers
					gSystemSettings.calibrationData.maxValues[i][j] = 0x0000ffff;
				} else
				{
					gSystemSettings.calibrationData.minValues[i][j] = 0x0000ffff;		// Sliders
					gSystemSettings.calibrationData.maxValues[i][j] = 0x00000000;
				}
				gSystemSettings.calibrationData.maskFlags[i][j] = true;
			}
		}

		// Mark the unused multiplexer inputs
		gSystemSettings.calibrationData.maskFlags[0][(uint32_t)eMultiplexerChannel_0::P_NC_1] = false;
		gSystemSettings.calibrationData.maskFlags[0][(uint32_t)eMultiplexerChannel_0::P_NC_2] = false;
		gSystemSettings.calibrationData.maskFlags[0][(uint32_t)eMultiplexerChannel_0::P_NC_3] = false;
		gSystemSettings.calibrationData.maskFlags[0][(uint32_t)eMultiplexerChannel_0::P_NC_4] = false;
...
void UI_Thread(void* pvParameters)
{
	...
	oledDriverPtr->drawText(10, 20, "Formating Flash", &CasualNoises::font_7x10);
...
//	deleteAllTLVs ( driverQueueHandle );
	...
	oledDriverPtr->drawText ( 20, 25, "Fellhorn", &CasualNoises::font_11x18 );
	oledDriverPtr->drawText ( 60, 46, "Rev #0.2", &CasualNoises::font_7x10 );
	oledDriverPtr->refreshDisplay ();
